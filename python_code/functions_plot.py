...
def load_metric(metric:str,params:dict,n_freeriders:int,
    experiment_specific:str):
    
    path=f"hist/{metric}/{params['dataset']}_{params['solver']}"
    
    FL=pickle.load(open(f'{path}_FL_{experiment_specific}.pkl', 'rb'))
    plain=pickle.load(open(f"{path}_plain_{n_freeriders}_{experiment_specific}"
        ".pkl", 'rb'))
    disg_1_1=pickle.load(open(f"{path}_disguised_1.0_{params['std_0']}_"
        f"{n_freeriders}_{experiment_specific}_1.pkl", 'rb'))    
    disg_3_1=pickle.load(open(f"{path}_disguised_1.0_{params['std_0']}_"
        f"{n_freeriders}_{experiment_specific}_3.pkl", 'rb'))
    disg_1_05=pickle.load(open(f"{path}_disguised_0.5_{params['std_0']}_"
        f"{n_freeriders}_{experiment_specific}_1.pkl", 'rb'))    
    disg_3_05=pickle.load(open(f"{path}_disguised_0.5_{params['std_0']}_"
        f"{n_freeriders}_{experiment_specific}_3.pkl", 'rb'))
    disg_1_2=pickle.load(open(f"{path}_disguised_2.0_{params['std_0']}_"
        f"{n_freeriders}_{experiment_specific}_1.pkl", 'rb'))    
    disg_3_2=pickle.load(open(f"{path}_disguised_2.0_{params['std_0']}_"
        f"{n_freeriders}_{experiment_specific}_3.pkl", 'rb'))
...
def get_exp_specific(d:dict,n_fr:int):
    ...
    if (d["dataset"]=="MNIST-iid" or d["dataset"]=="MNIST-shard" 
        or d["dataset"]=="CIFAR-10"):
        return (f"{d['n_clients']}_{d['samples']}"
            +f"_{d['epochs']}_{n_iter}"+f"_{d['lr']}")

    elif d["dataset"]=="shakespeare":
        return (f"{d['n_clients']}"
            +f"_{d['epochs']}_{n_iter}"+f"_{d['lr']}")
...
def figure_params(params_type:str, dataset:str, n_epochs:int, solver:str):
    ...
    if params_type=="loss" or params_type=="acc":
        d=[load_metric(params_type, params, n_fr, get_exp_specific(params, n_fr))
            for n_fr in [1,5,45]]
    ...
    def plot_dic(dic:dict, hist:list, plot_idx:int, metric:str):
        ...
        if metric == "acc":
            ...
            try:
                ...
                ax.fill_between(x,hist[0][:len(dic["FL"])], hist[1][:len(dic["FL"])], 
                    alpha=0.3)
            except:
        ...
        if plot_idx==0:ax.legend(loc="best",ncol=2)
...
def plot_metric_n_fr(list_columns:list, list_hist:list,
        metric:str,y_label:str,title:str,save_name:str, cols:list, y_min:list,
        log_scale=False):
    ...
    def plot_dic(dic:dict, hist:list, plot_idx:int, metric:str):
        ...
        if metric == "acc":
            ...
            try:
                ...
                ax.fill_between(x,hist[0][:len(dic["FL"])], 
                    hist[1][:len(dic["FL"])], alpha=0.3)
            except:
                if plot_idx//len(cols)==0:
                    print(f"NO AVAILABLE HISTORY FOR {cols[plot_idx%len(cols)]}")
        ...
        ax.plot(dic["disg_1_05"], label=r"Disguised $\sigma$, $\gamma=0.5$", 
            linestyle='--')
        ax.plot(dic["disg_3_05"], label=r"Disguised $3\sigma$, $\gamma=0.5$",
            linestyle='--')
        
        ax.plot(dic["disg_1_2"], label=r"Disguised $\sigma$, $\gamma=2$",
            linestyle='--')
        ax.plot(dic["disg_3_2"], label=r"Disguised $3\sigma$, $\gamma=2$",
            linestyle='--')
        ...
        if plot_idx==0 and metric=="acc": ax.legend(loc="lower right",ncol=2)
        elif plot_idx==0 and metric=="loss": ax.legend(loc="upper right",ncol=2)
...
def plot_metric_history(solver:str, n_epochs:int, metric:str, log_scale=False):
    ...
    if log_scale: save_name+="_log"
    
    plot_metric_n_fr(cols, intervals, metric,"Accuracy", "",
        save_name= save_name, cols=cols_names, y_min=y_min,log_scale=log_scale)
