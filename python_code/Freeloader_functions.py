...
def get_n_params(model):
    ...
    n_params=sum([np.prod(tensor.size()) 
        for tensor in list(model.parameters())])
    return n_params



def linear_noising(model, list_std:list, list_power:list, iteration:int, 
    noise_type:str, std_multiplicator):
    ...
    if noise_type=="disguised":
        for idx,layer_tensor in enumerate(model.parameters()):
            ...
            std_tensor=(torch.zeros(layer_tensor.size())
...
def FL_freeloader(n_freeriders:int, global_model, training_dls:list, 
        n_fr_samples:int, n_iter:int, testing_set:list, loss_f, device:str,
        mu:float, file_root_name:str, multiplicator:int, noise_type="plain",
        epochs=5, lr=10**-4, std_0=0, power=1, decay=1):
    """
    Parameters:
    ...
    from FL_functions import (loss_dataset,accuracy_dataset,local_learning,
    ...
    weights=[n_fr_samples/n_samples]*n_freeriders+[len(db.dataset)/n_samples 
        for db in training_dls]
    print("clients' weights:",weights)
    
    
    loss_hist=[[float(loss_dataset(global_model,training_dls[k],loss_f).detach()) 
        for k in range(K)]]
    acc_hist=[[accuracy_dataset(global_model,testing_set[k]) for k in range(K)]]
    server_hist=[[tens_param.detach().numpy() 
        for tens_param in list(global_model.parameters())]]
    
    #NOISE PARAMETERS
    if noise_type=="plain":list_std=[0,0]
    elif noise_type=="disguised":list_std=[0,std_0]
    list_power=[0,power]
    
    
    server_loss=sum([weight*loss_hist[-1][idx] 
        for idx,weight in enumerate(weights_fair)])
    server_acc=sum([weight*acc_hist[-1][idx] 
        for idx,weight in enumerate(weights_fair)])
    ...
    for i in range(n_iter):
        ...
        for j in range(n_freeriders):
            ...
            local_model=linear_noising(deepcopy(global_model), list_std, 
                list_power, max(i,1), noise_type, multiplicator).to(device)
        ...
        for k in range(K):
            ...
            local_loss=local_learning(local_model,mu,local_optimizer,training_dls[k],
        ...
        new_model=FedAvg_agregation_process(deepcopy(global_model),clients_params,device,
            weights=weights).cpu()
