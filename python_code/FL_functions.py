...
def local_learning(model,mu:float,optimizer,train_data,epochs:int,
        loss_f,device:str):
    
    model_0=deepcopy(model)
    
    for e in range(epochs):
        local_loss=train_step(model,model_0,mu,optimizer,train_data,loss_f,device)
...
def FedProx(model, training_sets:list, n_iter:int, loss_f, testing_set:list, 
        device:str, mu:float, file_root_name:str, epochs=5,lr=10**-4,decay=1):
    """
    ...
    loss_hist=[[float(loss_dataset(model,training_sets[k],loss_f).detach()) 
        for k in range(K)]]
    ...
    for i in range(n_iter):
        ...
        for k in range(K):
            ...
            local_loss=local_learning(local_model,mu,local_optimizer,
                training_sets[k],epochs,loss_f,device)
        ...
        model=FedAvg_agregation_process(deepcopy(model),clients_params,device, 
            weights=weights).cpu()
