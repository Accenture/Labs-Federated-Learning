...
# from FL.file_name import get_file_name
...
# ):
#     """
#     returns the history of the simulation associated to the given inputs
#     """
#     from FL.file_name import get_file_name
#
...
#         print(file_name)
#
#     with open(f"./saved_exp_info/{metric}/{file_name}.pkl", "rb") as output:
...
def hist(
    ...
    **args,
    ...
    l_args = [args.copy() for _ in range(len(seeds))]
    for idx, s in enumerate(seeds):
        l_args[idx]["seed"] = s

    l_exp = [Experiment(a, training=False, verbose=False) for a in l_args]
    for exp in l_exp:
        exp.hists_init()

    if mean:
        mean = np.mean([getattr(exp, metric) for exp in l_exp], axis=0)
        return mean
    else:
        return np.array([getattr(exp, metric) for exp in l_exp])
...
def get_lemma_1_quadra(
    ...
    v = dist(args.theta0, compared_model)
    v += (
        -2
        * args.eta_g
        * phi
        * (args.theta0 - glob_min)
        .reshape(-1)
        .dot((args.theta0 - compared_model).reshape(-1))
    )
    v += args.eta_g ** 2 * phi ** 2 * gamma.dot(dist_local_opt(args.theta0, loc_min))
    v += args.eta_g ** 2 * (1 + alpha) * phi ** 2 * dist_opt(args.theta0, glob_min)
